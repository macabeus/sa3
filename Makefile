...
# Needs a host arm-none-eabi toolchain; see tools/asmlift-sidecar.sh.
#
# The sidecar TU is GENERATED from the header list below (a build artifact, like the
# rest of build/). The list is CURATED, not a glob: it starts from the umbrella headers
# and grows as more globals matter to the benchmark.
...
# Globals the sidecar must also carry that no header declares (defined at file scope in
# upstream .c files). Linkage here is irrelevant — only the DWARF type DIEs are consumed.
define ASMLIFT_CTX_EXTRAS
/* file-scope globals no header declares (src/game/math.c, src/sprite.c) */
extern u32 gRngValue;
extern u32 gRngPrevValue;
extern const u8 unkFractions[8];
endef
export ASMLIFT_CTX_EXTRAS
